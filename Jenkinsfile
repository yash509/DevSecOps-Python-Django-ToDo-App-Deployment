pipeline {
    ...
    stages {
        ...
        stage("Sonarqube Analysis ") {                         
            steps {
                //dir('Band Website') {
                    ...
                    sh '''$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=django-application \
                    -Dsonar.projectKey=django-application'''
                    //}
                }
            }
        }
        ...
        stage("Docker Image Tagging"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                            sh "docker tag django-todo yash5090/django-todo:latest " 
                        //}
                    }
                }
            }
        ...
        stage('Docker Image Scanning') { 
            steps { 
                sh "trivy image --format table -o trivy-image-report.html yash5090/django-todo:latest" 
            } 
        } 
        ...
        stage("Image Push to DockerHub") {
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                            sh "docker push yash5090/django-todo:latest "
                        //}
                    }
                }
            }
        ...
        stage('Docker Scout Image Scanning') {
            steps {
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       sh 'docker-scout quickview yash5090/django-todo:latest'
                       sh 'docker-scout cves yash5090/django-todo:latest'
                       sh 'docker-scout recommendations yash5090/django-todo:latest'
                       sh 'docker-scout attestation yash5090/django-todo:latest'
                   }
                }   
            }
        }
        ...
        stage("TRIVY"){
            steps{
                //dir('Band Website') {
                    sh "trivy image yash5090/django-todo:latest > trivyimage.txt"   
                //}
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop django-todo | true"
                sh "docker rm django-todo | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh 'docker run -d --name django-notes -p 8000:8000 yash5090/django-todo:latest' 
                //}
            }
        }
